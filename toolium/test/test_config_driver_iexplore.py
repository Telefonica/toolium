...
@mock.patch('toolium.config_driver.IeOptions')
@mock.patch('toolium.config_driver.IeService')
@mock.patch('toolium.config_driver.webdriver')
def test_create_local_driver_iexplore_no_driver_path(webdriver_mock, service, options, config, utils):
    ...
    webdriver_mock.Ie.assert_called_once_with(service=service(), options=options())


@mock.patch('toolium.config_driver.IeOptions')
@mock.patch('toolium.config_driver.IeService')
@mock.patch('toolium.config_driver.webdriver')
def test_create_local_driver_iexplore_driver_path(webdriver_mock, service, options, config, utils):
    config.set('Driver', 'type', 'iexplore')
...
def test_get_iexplore_options(config, utils):
    config_driver = ConfigDriver(config, utils)
    expected_arguments = []
    expected_capabilities = DEFAULT_CAPABILITIES

    options = config_driver._get_explorer_options()

    assert options.arguments == expected_arguments
    assert options.capabilities == expected_capabilities


def test_get_iexplore_options_capabilities_update(config, utils):
...
def test_get_iexplore_options_capabilities_new(config, utils):
    config.add_section('Capabilities')
...
def test_get_iexplore_options_with_previous_capabilities(config, utils):
    config_driver = ConfigDriver(config, utils)
    expected_arguments = []
    expected_capabilities = DEFAULT_CAPABILITIES.copy()
    ...
    options = config_driver._get_explorer_options(previous_capabilities)
...
@mock.patch('toolium.config_driver.webdriver')
def test_create_remote_driver_iexplore_basepath(webdriver_mock, config, utils):
    config.set('Driver', 'type', 'iexplore')
...
@mock.patch('toolium.config_driver.webdriver')
def test_create_remote_driver_iexplore_with_version_and_platform(webdriver_mock, config, utils):
    config.set('Driver', 'type', 'iexplore-50-on-linux')
    server_url = 'http://10.20.30.40:5555'
    utils.get_server_url.return_value = server_url
    config_driver = ConfigDriver(config, utils)
    DriverWrappersPool.output_directory = ''
...
@mock.patch('toolium.config_driver.webdriver')
def test_create_remote_driver_iexplore_with_version_and_platform_uppercase(webdriver_mock, config, utils):
    config.set('Driver', 'type', 'iexplore-50-on-WIN10')
    server_url = 'http://10.20.30.40:5555'
    utils.get_server_url.return_value = server_url
    config_driver = ConfigDriver(config, utils)
    DriverWrappersPool.output_directory = ''
    expected_capabilities = DEFAULT_CAPABILITIES.copy()
    expected_capabilities['browserVersion'] = '50'
    expected_capabilities['platformName'] = 'win10'
