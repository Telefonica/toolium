...
def test_change_jira_status(logger):
    ...
    jira.logger.info.assert_called_with("Created execution TOOLIUM-TEST for test TOOLIUM-1")
...
def test_change_jira_status_empty_url(jira_get, logger):
    ...
    jira.logger.warning.assert_called_once_with(
        "Test Case '%s' can not be updated: execution_url is not configured", 'TOOLIUM-1')
