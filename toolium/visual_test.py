...
class VisualTest:
    ...
    results = None  #: dict to save visual assert results
    ...
    def assert_screenshot(self, element, filename, file_suffix=None, threshold=0, exclude_elements=None):
        ...
        if not (isinstance(threshold, int) or isinstance(threshold, float)) or threshold < 0 or threshold > 1:
            raise TypeError(f'Threshold must be a number between 0 and 1: {threshold}')

        # Search elements
        web_element = self.utils.get_web_element(element)
        exclude_web_elements = self._get_exclude_web_elements(exclude_elements)

        baseline_path = os.path.join(self.baseline_directory, f'{filename}.png')
        filename_with_suffix = f'{filename}__{file_suffix}' if file_suffix else filename
        unique_name = f'{DriverWrappersPool.visual_number:0=2d}_{filename_with_suffix}'
        unique_name = f'{get_valid_filename(unique_name)}.png'
        output_path = os.path.join(self.output_directory, unique_name)
        report_name = f'{file_suffix}<br>({filename})' if file_suffix else f'-<br>({filename})'
    ...
    def get_element_box(self, web_element):
        ...
        return (
            int(location['x']) + offset_x,
            int(location['y'] + offset_y),
        ...
        if web_elements and len(web_elements) > 0:
            ...
            for web_element in web_elements:
                ...
                for x, y in itertools.product(
                    range(element_box[0], element_box[2]),
    ...
    def compare_files(self, report_name, image_path, baseline_path, threshold):
        ...
        if image_size != baseline_size:
            ...
            exception_message = (
                f"The new screenshot '{image_path}' size '{image_size}' did not match the"
            ...
            exception_message = (
                f"The new screenshot '{image_path}' did not match the baseline '{baseline_path}'"
                f' (by a distance of {diff_pixels_percentage:.8f}, more than {threshold} threshold)'
            )
            result = 'diff'

        if (
            result == 'equal' and self.driver_wrapper.config.getboolean_optional('VisualTests', 'complete_report')
        ) or result == 'diff':
    ...
    def _add_summary_to_report(self):
        ...
        summary += f'<p><b>Baseline name</b>: {path.basename(self.baseline_directory)}</p>'
